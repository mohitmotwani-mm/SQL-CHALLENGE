use [financial analysis challenge]
...
select TOP 1 b.branchname,round(avg(a.balance),2)as avg_accountbalance from branches as b
join accounts as a on b.branchid = accountid
group by BranchName
order by  avg_accountbalance desc


--Which customer has the highest current balance in their accounts? 

select TOP 1 c.firstname,c.lastname,sum(a.balance)as current_balance from customers as c
join accounts as a on c.customerid = a.customerid
...
select TOP 1 c.customerid,c.firstname,count(TransactionID)as transaction_count from customers as c
join accounts as a on c.customerid = a.customerid
...
select TOP 1 b.branchname,sum(a.balance) as total_balance from branches as b
...
select b.branchname,count(TransactionID) as no_of_trans from branches as b
join accounts as a on b.branchid = a.branchid
join transactions as t on a.accountid = t.accountid
group by b.branchname
